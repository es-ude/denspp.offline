...
import denspp.offline.nsp.plot_nsp as nsp
import denspp.offline.pipeline.plot_pipeline as pipe
...
class Pipeline(PipelineCMD):
    ...
    def do_plotting(self, data: PipelineSignal, channel: int) -> None:
        ...
        pipe.plot_pipeline_afe(data, channel, path=path2save)
        pipe.plot_transient_highlight_spikes(data, channel, path=path2save)
        pipe.plot_transient_highlight_spikes(data, channel, path=path2save, time_cut=[10, 12])
        pipe.plot_pipeline_frame_sorted(data, channel, path=path2save)
        pipe.plot_pipeline_results(data, channel, path=path2save)

        # --- NSP block
        nsp.plot_nsp_ivt(data, channel, path=path2save)
        # nsp.plot_firing_rate(data, channel, path=path2save)
        # nsp.plot_nsp_correlogram(data, channel, path=path2save)
        # nsp.plot_nsp_cluster_amplitude(data, channel, path=path2save)
