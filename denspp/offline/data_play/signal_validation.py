...
class SignalComparator:
    ...
    def _calculate_pearson_korrelation(self) -> list:
        ...
        if not self._equal_number_of_signals:
            raise ValueError(
                "Original and processed signals must have the same length for similarity calculation."
            )
        for i, _ in enumerate(self._signal_original_with_cut):
            if len(self._signal_original_with_cut[i]) != len(self._signal_processed[i]):
                x_orig = np.linspace(
                    0, 1, len(self._signal_original_with_cut[i])
                )  # x Values for original signal
                x_proc = np.linspace(
                    0, 1, len(self._signal_processed[i])
                )  # x Values for processed signal

                processed_aligned = np.interp(
                    x_orig, x_proc, self._signal_processed[i]
                )  # Align processed signal to original signal length, e.g. Samplingrate
            else:
    ...
    def _calculate_mse(self) -> list:
        ...
        for i, _ in enumerate(self._signal_original_with_cut):
            ...
            orig_norm = (
                self._signal_original_with_cut[i] - np.min(self._signal_original_with_cut[i])
            ) / (np.max(self._signal_original_with_cut[i]) - np.min(self._signal_original_with_cut[i]))
            proc_norm = (processed_aligned - np.min(processed_aligned)) / (
                np.max(processed_aligned) - np.min(processed_aligned)
            )
    ...
    def _calculate_spectral_coherence(self, self_defined_nperseg=None) -> list:
        ...
        for i, _ in enumerate(self._signal_original_with_cut):
            ...
            f, Cxy = signal.coherence(
                self._signal_original_with_cut[i],
                processed_aligned,
                fs=self._fs_original,
                nperseg=nperseg,
            )
            self._plot_spectral_coherence(f, Cxy)
            self._show_signal_plots(
                self._signal_original_with_cut[i],
                processed_aligned / self._scaling_factor,
            )
