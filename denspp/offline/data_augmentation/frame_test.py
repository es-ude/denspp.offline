...
class TestCalculateWaveform(TestCase):
    def test_generate_zero_frames_float(self):
        rslt = generate_zero_frames(
            frame_size=32, num_frames=100, snr_range=[0, 20], fs=20e3, return_int=False
        )
    ...
    def test_generate_zero_frames_integer(self):
        rslt = generate_zero_frames(
            frame_size=32, num_frames=100, snr_range=[20, 40], fs=20e3, return_int=True
        )
        self.assertEqual(rslt[0].shape, (100, 32))
    ...
    def test_calculate_frames_mean(self):
        frames_zero = generate_zero_frames(
            frame_size=32, num_frames=10, snr_range=[0, 20], fs=20e3, return_int=True
        )
        dataset = {
        ...
        rslt = calculate_frame_mean(
            frames_in=dataset["data"], frames_cl=dataset["label"], return_int=False
        )
...
class TestAugmentationWaveform(TestCase):
    ...
    def test_augmentation_mean_waveform(self):
        ...
        np.testing.assert_array_equal(
            np.unique(rslt["id"], return_counts=True),
            (np.array([0, 1]), np.array([90, 96])),
        )
    ...
    def test_augmentation_change_position(self):
        ...
        np.testing.assert_array_equal(
            np.unique(rslt[1], return_counts=True),
            (np.array([0, 1]), np.array([40, 40])),
        )
    ...
    def test_augmentation_reduce_samples(self):
        ...
        np.testing.assert_array_equal(
            np.unique(rslt[1], return_counts=True), (np.array([0, 1]), np.array([4, 4]))
        )
