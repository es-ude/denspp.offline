...
class TestPolynomFitting(TestCase):
    ...
    def test_voltage_response(self):
        dut = PolyfitIV(sampling_rate=10e3, en_noise=False)
        dut.extract_polyfit_params(current=self.current_mea, voltage=self.voltage_mea)
    ...
    def test_get_params_single(self):
        ...
        error = dut.extract_polyfit_params(
            current=self.current_mea, voltage=self.voltage_mea, find_best_order=False
        )
