...
class PySpiceHandler:
    ...
    def _load_circuit_model(self) -> None:
        ...
        if self._settings.type in self._type_device.keys():
            if [key for key in self._settings.params_use.keys()] == self._type_device[
                self._settings.type
            ]["param"]:
                circuit_loaded = self._type_device[self._settings.type]["circ"](
                    **self._settings.params_use
                )
            else:
                raise NotImplementedError(
                    f"Model Params are unequal: Need {self._type_device[self._settings.type]['param']} is not supported"
                )
        else:
            raise NotImplementedError(
                f"Type {self._settings.type} is not supported (only {self._type_device.keys()})"
            )
    ...
    def print_types(self) -> list:
        ...
        for idx, type in enumerate(self._type_device.keys()):
            ...
            methods.append(
                f"{type} = {self._type_device[type]['desp']} (params = {self._type_device[type]['param']})"
            )
        return methods
    ...
    def do_dc_simulation(self, value: float, initial_value: float = 0.0) -> dict:
        ...
        simulator = self._circuit.simulator(
            temperature=self._settings.temp_celsius,
    ...
    def do_transient_pulse_simulation(
        self,
        neg_value: float,
        pos_value: float,
        pulse_width: float,
        ...
        self._load_circuit_model()
        if self._settings.input_volt:
            self._circuit.PulseVoltageSource(
                "input",
                "input",
                self._circuit.gnd,
                initial_value=neg_value,
                pulsed_value=pos_value,
                pulse_width=pulse_width,
                period=pulse_period,
                rise_time=1 / self._settings.fs_ana,
                fall_time=1 / self._settings.fs_ana,
            )
            self._circuit.Vinput.minus.add_current_probe(self._circuit)
        else:
            self._circuit.PulseCurrentSource(
                "input",
                self._circuit.gnd,
                "input",
                initial_value=neg_value,
    ...
    def do_transient_sinusoidal_simulation(
        self,
        amp: float,
        freq: float,
        t_sim: float,
    ...
    def do_transient_arbitrary_simulation(
        self,
        signal: np.ndarray,
        t_end: float,
        initial_value: float = 0.0,
        trans_value: float = 1.0,
    ) -> dict:
    ...
    def plot_iv_curve(self, do_log: bool = False, path2save: str = "", show_plot: bool = False) -> None:
        ...
        plt.xlabel(rf"Voltage $U$ / {units_u}V")
        plt.ylabel(rf"Current $I$ / {units_i}A")
    ...
    def plot_bodeplot(self, mode: int = 0, path2save: str = "", show_plot: str = False) -> None:
        ...
        freq = results["freq"]
        transfer_function = (
            results["v_out"] / results["v_in"] if mode == 0 else results["v_in"] / results["i_in"]
        )
        ...
        axs[0].semilogx(
            freq,
            20 * np.log10(np.abs(transfer_function)),
            "k",
            linewidth=1,
            marker=".",
            label="Gain",
        ...
        axs[1].semilogx(
            freq,
            np.angle(transfer_function, deg=True),
            "r",
            linewidth=1,
            marker=".",
            label="Phase",
    ...
    def plot_transient(self, path2save: str = "", show_plot: bool = False) -> None:
        ...
        ax1.set_ylabel(rf"Voltage $U_x$ / {units_u}V")
        ax1.set_xlabel(rf"Time $t$ / {units_t}s")

        ax2 = ax1.twinx()
        lns += ax2.plot(scale_t * time, scale_i * i_in, "r--", linewidth=1, label="Current")
        ax2.set_ylabel(rf"Current $I$ / {units_i}A")
...
def create_dummy_signal(
    ...
    f_samp: float,
    offset: float = 0.0,
    freq_used: list = [100, 300, 500],
    freq_amp: list = [1.0, 0.25, 0.66],
