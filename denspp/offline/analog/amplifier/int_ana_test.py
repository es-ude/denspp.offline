...
from .int_ana import IntegratorAmplifier, SettingsINT
...
@pytest.mark.parametrize("vss, vdd, vcm, vrange", [(-1.2, 1.2, 0.0, 2.4), (0.0, 1.8, 0.9, 1.8)])
...
class IntegratorAmplifierTest(TestCase):
    ...
    def test_ideal_integrator_sample_zeros(self):
        ...
        rslt = self.dut.do_ideal_integration_sample(u_inp=self.sig_zero, u_inn=TestSettingsINT.vcm)
        self.assertEqual(rslt, chck)

    def test_ideal_integrator_sample_ones(self):
        chck = 0.1
        rslt = self.dut.do_ideal_integration_sample(u_inp=self.sig_ones, u_inn=TestSettingsINT.vcm)
        self.assertLess(abs(rslt - chck), 1e-6)

    def test_ideal_integrator_sample_sym(self):
        chck = TestSettingsINT.vcm
        rslt = self.dut.do_ideal_integration_sample(u_inp=self.sig_sym, u_inn=TestSettingsINT.vcm)
        self.assertLess(abs(rslt - chck), 1e-4)

    def test_ideal_integrator_sample_clamp_vdd(self):
        chck = TestSettingsINT.vdd
        rslt = self.dut.do_ideal_integration_sample(u_inp=self.sig_ones * 10, u_inn=TestSettingsINT.vcm)
        self.assertLess(abs(rslt - chck), 1e-4)
    ...
    def test_ideal_integrator_signal_zeros(self):
        chck = self.sig_zero
        rslt = self.dut.do_ideal_integration(u_inp=self.sig_zero, u_inn=TestSettingsINT.vcm)
    ...
    def test_ideal_integrator_signal_ones(self):
        ...
        rslt = self.dut.do_ideal_integration(u_inp=self.sig_ones, u_inn=TestSettingsINT.vcm)
        np.testing.assert_almost_equal(rslt, chck, decimal=2)

    def test_ideal_integrator_clamp_vdd(self):
        chck = TestSettingsINT.vdd
        rslt = self.dut.do_ideal_integration(u_inp=self.sig_ones * 10, u_inn=TestSettingsINT.vcm)
    ...
    def test_opa_volt_integrator_zeros(self):
        ...
        rslt = self.dut.do_opa_volt_integration(
            u_inp=-self.sig_zero, u_inn=TestSettingsINT.vcm + TestSettingsINT.u_error
        )
        self.assertLess(float(sum(np.abs(rslt - chck))) / self.sig_zero.size, 1e-4)

    def test_opa_volt_integrator_ones(self):
        chck = (
            -np.linspace(start=0.0, stop=0.1, num=self.sig_ones.size, endpoint=True)
            + TestSettingsINT.vcm
            - TestSettingsINT.u_error
        )
        rslt = self.dut.do_opa_volt_integration(
            u_inp=-self.sig_ones, u_inn=TestSettingsINT.vcm + TestSettingsINT.u_error
        )
    ...
    def test_opa_curr_integrator_zeros(self):
        ...
        rslt = self.dut.do_opa_curr_integration(
            i_in=-self.sig_zero / TestSettingsINT.res_in, u_ref=TestSettingsINT.vcm
        )
        self.assertLess(float(sum(np.abs(rslt - chck))) / self.sig_zero.size, 1e-4)
    ...
    def test_opa_curr_integrator_ones(self):
        ...
        rslt = self.dut.do_opa_curr_integration(
            i_in=-self.sig_ones / TestSettingsINT.res_in, u_ref=TestSettingsINT.vcm
        )
        self.assertLess(float(sum(np.abs(rslt - chck))) / self.sig_ones.size, 1e-3)
    ...
    def test_cap_curr_integrator_zeros(self):
        ...
        rslt = self.dut.do_cap_curr_integration(i_in=-self.sig_zero / TestSettingsINT.res_in)
        self.assertLess(float(sum(np.abs(rslt - chck))) / self.sig_zero.size, 1e-4)

    def test_cap_curr_integrator_ones(self):
        chck = (
            np.linspace(start=0.0, stop=0.1, num=self.sig_ones.size, endpoint=True)
            * TestSettingsINT.tau_active_scale
        )
        chck += TestSettingsINT.vcm + TestSettingsINT.vss

        rslt = self.dut.do_cap_curr_integration(i_in=self.sig_ones / TestSettingsINT.res_in)
        self.assertLess(float(sum(np.abs(rslt - chck))) / self.sig_ones.size, 1e-3)
