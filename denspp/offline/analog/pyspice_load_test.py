...
class TestPySpiceLoad(TestCase):
    ...
    def test_dc_simulation_sweep_resistor(self):
        set0 = deepcopy(test_sets)
        val = PySpiceLoad(set0).do_dc_sweep_simulation(-1.0, 1.0, 0.5)
    ...
    def test_ac_simulation_voltage_divider(self):
        ...
        np.testing.assert_almost_equal(
            val["v_out"], np.abs(np.array([0.5, 0.5, 0.5, 0.5, 0.5])), decimal=6
        )
    ...
    def test_transient_arbitrary_voltage(self):
        ...
        time, sig = create_dummy_signal(
            100 / set0.fs_ana, set0.fs_ana, freq_used=[set0.fs_ana / 20], freq_amp=[1.0]
        )
