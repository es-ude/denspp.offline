...
def do_quantize_transient(transient_orig: dict, fs_new: float, u_lsb: float, i_gain: float = 2e3) -> dict:
    ...
    assert check_keylist_elements_all(
        keylist=[key for key in transient_orig.keys()], elements=["V", "I", "fs"]
...
def do_resample_time(
    signal_in: np.ndarray, fs_orig: float, fs_new: float, do_offset_comp: bool = False
) -> np.ndarray:
...
def calculate_signal_integration(
    signal: np.ndarray, time: np.ndarray, initial: float = 0.0
) -> np.ndarray:
    ...
    return initial + cumulative_trapezoid(y=signal, x=time, dx=period, initial=0.0, axis=-1)
