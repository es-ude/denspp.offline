...
class ElectricalLoad(ElectricalLoadHandler):
    ...
    def _func2cur_resistive_diode(
        self, i_path: np.ndarray, uth0: float, r_sh: float, n_eff: float, i_sat: float
    ) -> np.ndarray:
        ...
        return (
            uth0
            + r_sh * i_path
            + n_eff * self._settings_device.temperature_voltage * np.log(i_path / i_sat + 1)
        )
