...
class SpikeAnalysisTest(unittest.TestCase):
    ...
    def setUp(self):
        self.fs = 20e3
        self.dataset = _build_eap_dataset(count=12, sampling_rate=self.fs, max_gap=0.1, do_plot=False)
    ...
    def test_calc_amplitude(self):
        rslt = calc_amplitude(frames_in=self.dataset)
    ...
    def test_calc_spiketicks(self):
        rslt = calc_spike_ticks(spike=self.dataset)
    ...
    def test_calc_autocorrelegram(self):
        ...
        rslt = calc_autocorrelogram(ticks=ticks, fs=self.dataset.sampling_rate)
        self.assertEqual(len(rslt), 2)
    ...
    def test_calc_firing_rate(self):
        ticks = calc_spike_ticks(spike=self.dataset)
        rslt = calc_firing_rate(ticks=ticks, fs=self.fs)
        self.assertEqual(len(rslt), 2)
    ...
    def test_calc_interval_timing(self):
        ticks = calc_spike_ticks(spike=self.dataset)
        rslt = calc_interval_timing(ticks=ticks, fs=self.fs)
        self.assertEqual(len(rslt), 2)
