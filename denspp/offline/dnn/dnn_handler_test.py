...
from .dnn_handler import PyTorchTrainer, TrainingResults

# TODO: Disable plot block option during testing
# TODO: Add all custom metrics to testing
class TestPytorchTrainer(TestCase):
    ...
    def test_check_config_available(self):
        self.assertTrue((self.dut.path2config / "ConfigClassifier_MNIST.yaml").exists())
        self.assertTrue((self.dut.path2config / "ConfigDataset_MNIST.yaml").exists())
        self.assertTrue((self.dut.path2config / "ConfigTraining_MNIST.yaml").exists())
    ...
    def test_start_training_with_init(self):
        rmtree(self.dut.path2config)
        dut = PyTorchTrainer(
            usecase_name='MNIST',
            path2config='temp_test',
    ...
    def test_start_training_mnist_classifier(self):
        ...
        try:
            ...
            self.assertTrue(check_keylist_elements_all(list(rslt.data.keys()), ['cl_dict', 'input', 'valid_clus']))
