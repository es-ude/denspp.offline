...
def plot_common_loss(metric: dict, path2save: str = "", show_plots: bool = False) -> None:
    ...
    feat_size_ticks = (
        feat_size
        if len(feat_size) < 6
        else np.linspace(feat_size[0], feat_size[-1], 11, endpoint=True, dtype=np.uint16)
    )
...
def plot_architecture_violin(
    metric: dict, path2save: str = "", show_plots: bool = False, label_dict=None
) -> None:
    ...
    for key0, key1 in zip(keys_ae, keys_cl):
        ...
        for idx, data_boxplot in enumerate(transformed_data_ae_cluster):
            axs[0].violinplot(
                data_boxplot,
                showmedians=True,
                positions=np.array(feat_size) + idx / num_cluster,
                widths=0.5 / num_cluster,
            )
...
def plot_architecture_metrics_isolated(
    metric: dict, path2save: str = "", show_plots: bool = False, label_dict=None
) -> None:
    ...
    for key0 in keys_ae:
        ...
        for idx, data_boxplot in enumerate(transformed_data_ae_cluster):
            axs[int(idx / num_cols), idx % num_cols].plot(
                feat_size, transformed_data_ae_median[idx, :], "k.--", linewidth=1.0
            )
            axs[int(idx / num_cols), idx % num_cols].violinplot(
                data_boxplot, showmedians=True, positions=feat_size
            )
            axs[int(idx / num_cols), idx % num_cols].set_ylabel(
                f"{key0} ({label_dict[idx]})", fontsize=14
            )
