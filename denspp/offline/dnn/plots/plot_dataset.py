...
def plot_frames_dataset(
    data: DatasetFromFile,
    take_samples: int = 500,
    do_norm: bool = False,
    ...
    fig, axs = plt.subplots(
        ...
        sharex=True,
        sharey=True,
        figsize=(cm_to_inch(14), cm_to_inch(16)),
    )
    for id, key in enumerate(frame_dict):
        ...
        scale = (
            1
            if not do_norm
            else np.repeat(np.expand_dims(frame_peak[pos_random], -1), frame_raw.shape[-1], -1)
        )
...
def plot_waveforms_dataset(
    ...
    for idx in class_id:
        ...
        sample_idx = np.unique(
            np.random.randint(low=val_range[0], high=val_range[-1], size=num_samples_class)
        )
        axs[num_column, idx % axs.shape[1]].plot(
            dataset.data[sample_idx, :].T, label=dataset.dict[idx], color="gray"
        )
        axs[num_column, idx % axs.shape[1]].plot(
            np.mean(dataset.data[val_range, :], axis=0),
            label=dataset.dict[idx],
            color="k",
        )
