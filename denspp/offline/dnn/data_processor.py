...
class DataProcessor:
    ...
    def exclude_cluster_from_dataset(self, dataset: DatasetFromFile) -> DatasetFromFile:
        ...
        return DatasetFromFile(data=data_in, label=data_cl, dict=data_na, mean=data_me)

    def reconfigure_cluster_with_cell_lib(
        self, sel_mode_classes: int, dataset: DatasetFromFile
    ) -> DatasetFromFile:
        ...
        libs_class_overview = [
            lib.split("resort_")[-1] for lib in cell_libs_handler.get_library_overview(do_print=False)
        ]
        libs_use = [
            f"resort_{lib}" for lib in libs_class_overview if lib in self._settings.data_type.lower()
        ]
        if len(libs_use):
            cl_sampler = CellSelector(
                cell_merge=cell_libs_handler.build(libs_use[0]), mode=sel_mode_classes
            )
    ...
    def process_vision_datasets(self, data: DatasetFromFile) -> DatasetFromFile:
        ...
        if self._settings.use_cell_sort_mode:
            data_used = self.reconfigure_cluster_with_cell_lib(
                sel_mode_classes=self._settings.use_cell_sort_mode, dataset=data
            )
        ...
        if self._settings.reduce_samples_per_cluster_do:
            raise NotImplementedError(
                "No reducing samples technique is implemented - Please disable flag!"
            )
        # --- Return
        return data_used

    def process_timeseries_datasets(
        self, data: DatasetFromFile, add_noise_cluster: bool = False
    ) -> DatasetFromFile:
        ...
        if self._settings.normalization_do:
            ...
            self._logger.info("... do data normalization")
        ...
        if add_noise_cluster:
            ...
            new_frames, new_clusters, new_mean = generate_zero_frames(
                data_used.data.shape[1], num_frames, snr_range_zero
            )
            ...
            self._logger.info(
                f"... adding a zero-noise cluster: cluster = {num_cluster} - number of frames = {num_frames}"
            )
        return data_used
