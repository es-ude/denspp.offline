...
from denspp.offline.pipeline.pipeline_cmds import (
    DataloaderLibrary,
    PipelineCMD,
    PipelineLibrary,
)
...
class TestPipeCommands(TestCase):
    ...
    def test_pipecmd_generate_folder(self):
        self.dut.generate_run_folder(path2runs=join("temp_test", "runs"), addon="_test")
        self.assertTrue(exists(self.dut.path2save))

    def test_pipecmd_save_results(self):
        self.dut.generate_run_folder(path2runs=join("temp_test", "runs"), addon="_test")
        self.dut.save_results(name="test", data=dict({"name": "A", "value": 1}))
        self.assertTrue(exists(self.dut.path2save))
    ...
    def test_pipecmd_apply_mapping_shape_wo_zero(self):
        ...
        chck = data_mapd.shape == (2, 2, int(test_data.fs_orig)) and np.sum(data_mapd == 0.0) == 0
    ...
    def test_pipecmd_apply_mapping_shape_with_zero(self):
        ...
        chck = (
            data_mapd.shape == (2, 2, int(test_data.fs_orig))
            and np.sum(data_mapd == 0.0) == test_data.data_raw.shape[-1]
        )
        self.assertTrue(chck)
    ...
    def test_pipecmd_return_mapping_shape_wo_zero(self):
        ...
        data_ret = self.dut.deploy_mapping(
            data=data_mapd, electrode_id=test_data.electrode_id, mapping=mapping
        )
        chck = data_ret.shape == (4, int(test_data.fs_orig)) and np.sum(data_mapd == 0.0) == 0
        self.assertTrue(chck)
    ...
    def test_pipecmd_return_mapping_shape_with_zero(self):
        ...
        chck = data_ret.shape == (3, int(test_data.fs_orig)) and np.sum(data_mapd == 0.0) == int(
            test_data.fs_orig
        )
        self.assertTrue(chck)
