...
class _ThreadProcess(Thread):
    ...
    def run(self) -> None:
        self.results = self.pipeline(self.input)
...
class ProcessingThread:
    ...
    def __perform_single_threads(self, func, rawdata: np.ndarray, chnnl_id: list) -> None:
        ...
        for idx, (chnnl, data)  in enumerate(tqdm(zip(chnnl_id, rawdata), ncols=100, desc='Progress: ')):
            self.__threads_worker.append(_ThreadProcess(
                rawdata=data,
                func=func
            ))
            self.__threads_worker[idx].start()
            self.__threads_worker[idx].join()
            self._results.update({f'{chnnl}': self.__threads_worker[idx].results})

    def __perform_multi_threads(self, func, data: np.ndarray, chnnl_id: list) -> None:
        ...
        for group in tqdm(split_groups, ncols=100, desc='Progress: '):
            ...
            for idx, elec in enumerate(group):
                self.__threads_worker[idx].join()
                self._results.update({f'{elec}': self.__threads_worker[idx].results})
    ...
    def do_processing(self, func, data: np.ndarray, chnnl_id: list) -> None:
