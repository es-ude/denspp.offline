...
class PipelineLibrary:
    ...
    def get_registry(self, package: str = "src_pipe") -> ModuleRegistryManager:
        ...
        m.register_package(package) if exists(join(project_root, package)) else m.register_package(
            "denspp.offline.template"
        )
...
class PipelineCMD:
    ...
    def apply_mapping(self, data: np.ndarray, electrode_id: list, mapping: np.ndarray) -> np.ndarray:
        ...
        assert np.count_nonzero(mapping) == len(electrode_id), (
            "No mapping is available or mapping content does not match electrode ID"
        )
    ...
    def deploy_mapping(self, data: np.ndarray, electrode_id: list, mapping: np.ndarray) -> np.ndarray:
        ...
        assert data.shape[0] * data.shape[1] >= len(electrode_id), (
            "Mismatch between electrode_id and data shape"
        )
