...
def _build_spike_waveform(sampling_rate: float, do_plot: bool = False) -> np.ndarray:
    eap = WaveformGenerator(sampling_rate=sampling_rate, add_noise=False).generate_waveform(
        time_points=[0.0],
...
def _build_spike_signal(
    ...
    if do_plot:
        ...
        plt.plot(
            time[spike_pos.tolist()],
            spike_signal[spike_pos.tolist()],
            "r",
            marker=".",
            linestyle="None",
        )
...
class TestBuildEAP(unittest.TestCase):
    def test_build_spike_waveform(self):
        signal = _build_spike_waveform(sampling_rate=20e3, do_plot=False)
...
class TestFrameGenerator(unittest.TestCase):
    def setUp(self):
        ...
        self.frames_eap = np.array(
            [
                _build_spike_waveform(sampling_rate=self.set0.sampling_rate, do_plot=False)
                for _ in range(10)
            ]
        )
    ...
    def test_frame_generation_transient_moving_average(self):
        ...
        self.assertEqual(rslt.waveform.shape[1], self.set0.length_frame_int)
        self.assertGreaterEqual(rslt_pos.f1_score, 0.83)
        self.assertEqual(
            np.argmin(rslt.waveform, axis=1).tolist(),
            [self.set0.length_align_position for _ in range(rslt.waveform.shape[0])],
        )
