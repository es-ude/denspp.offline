...
from denspp.offline.data_call.call_handler import ControllerData
...
class PlayerControllerData(ControllerData):
    ...
    def syntheticSineWave(self) -> None:
        ...
        time_points = np.linspace(
            0, sine_t_sim, total_pointsX, endpoint=False
        )  # with endpoint False the last point is excluded; all points have same distance to each other
    ...
    def eeg_mental_arithemtic_task(self) -> None:
        ...
        path_to_file = self._prepare_access_file(
            folder_name="eeg_mental_arithemtic_task", data_type="s*.csv"
        )
