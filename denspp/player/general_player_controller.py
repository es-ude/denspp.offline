...
from .output_devices import (
    hardware_specification_denspp_player,
    hardware_specification_oscilloscope_mox4,
)
...
class GeneralPlayerController:
    ...
    def _read_config(self) -> None:
        ...
        self._data_config_do_cut = self.general_config["Data_Configuration"]["Data_Preprocessing"][
            "do_cut"
        ]
        self._data_config_do_resample = self.general_config["Data_Configuration"]["Data_Preprocessing"][
            "do_resampling"
        ]
        self._translation_value_voltage = self.general_config["Data_Configuration"]["Voltage_Scaling"][
            "translation_value_voltage"
        ]
        self._hardware_data_channel_mapping = self.general_config["Data_Configuration"][
            "Hardware_Data_Mapping"
        ]["channel_mapping"]
    ...
    def _config_hardware_controller(self) -> DataTranslator:
        ...
        for (
            ...
            self._hardware_config_values.items()
        ):  # Check for multiple used devices, this is not allowed, and get the used device
            if data["used"]:
        ...
        if used_device[0] == "DensPPPlayer":
            ...
            self._logger.CRITICAL(
                f"The specified hardware device '{used_device[0]}' is not defined in output_devices.py."
            )
            raise Exception(
                f"The specified hardware device '{used_device[0]}' is not defined in output_devices.py."
            )

        hardware_controller = DataTranslator(
            specific_device_settings=specific_device_settings,  # Create hardware controller with specific device settings
            logger=self._logger,
    ...
    def _config_call_handler_SettingsData(self) -> SettingsData:
        ...
        """
        deployed_settingsData = SettingsData(
            pipeline="PipelineV0",
            do_merge=False,
            ...
            t_range_sec=[
                self._data_config_values["Time"]["start_time"],
                self._data_config_values["Time"]["end_time"],
            ]
            if self._data_config_values["Time"]["start_time"] is not None
            and self._data_config_values["Time"]["end_time"] is not None
            else [],
            do_mapping=False,
    ...
    def _resample_data(self) -> None:
        ...
        """
        if (
            self._deployed_playerControllerData._settings.fs_resample
            > self._deployed_hardware_controller._dac_max_sampling_rate
        ):  # Check if the desired sampling rate is supported by the hardware
            self._logger.CRITICAL(
                f"The desired resampling rate of {self._deployed_playerControllerData._settings.fs_resample} Hz exceeds the maximum supported rate of {self._deployed_hardware_controller._dac_max_sampling_rate} Hz for the selected hardware."
            )
            raise Exception(
                f"The desired resampling rate of {self._deployed_playerControllerData._settings.fs_resample} Hz exceeds the maximum supported rate of {self._deployed_hardware_controller._dac_max_sampling_rate} Hz for the selected hardware."
            )
        ...
        if self._data_config_do_resample:
            ...
            self._logger.info(
                f"Data resampling completed, new sampling rate: {self._deployed_playerControllerData._raw_data.fs_used} Hz"
            )
        else:
    ...
    def _analyze_signals(self) -> None:
        ...
        compartor = SignalCompartor(
            original_data_with_cut=original_data_with_cut.data_raw,
