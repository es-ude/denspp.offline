...
from .output_devices import (
    ...
    hardware_specification_denspp_player_import,
    hardware_specification_denspp_player_sdcard,
...
class GeneralPlayerController:
    ...
    def __init__(self, class_config: DensppGenerationPlayerConfig = None):
        if class_config:
            ...
            self._cut_data()
            self._untreated_raw_data_with_cut = copy.deepcopy(
                self._deployed_playerControllerData.get_data()
            )
            self._resample_data()
    ...
    def _read_config(self) -> None:
        ...
        return DensppGenerationPlayerConfig(
            logging_lvl=loaded_config["General_Configuration"]["Logging_Lvl"],
    ...
    def _resample_data(self) -> None:
        ...
        if (
            ...
            > self._deployed_data_translator._dac_max_sampling_rate
        ):  # Check if the desired sampling rate is supported by the hardware
            self._logger.CRITICAL(
                f"The desired resampling rate of {self._deployed_playerControllerData._settings.fs_resample} Hz exceeds the maximum supported rate of {self._deployed_data_translator._dac_max_sampling_rate} Hz for the selected hardware."
            )
            raise Exception(
                f"The desired resampling rate of {self._deployed_playerControllerData._settings.fs_resample} Hz exceeds the maximum supported rate of {self._deployed_data_translator._dac_max_sampling_rate} Hz for the selected hardware."
            )
        ...
        if self._player_config.do_resample:
            ...
            if (
                self._player_config.logging_lvl.upper() == "DEBUG"
