...
module FIR_HALF_{$device_id}#(
    ...
    localparam STATE_IDLE = 2'b00, STATE_PREP = 2'b01, STATE_FIR_FORW = 2'b10, STATE_FIR_BACK = 2'b11;
    
    //Control Signals
    reg [1:0] state;
    reg [$clog2(LENGTH)-'d1:0] cnt_adr_wght;
    reg [$clog2(2*LENGTH-'d1)-'d1:0] cnt_adr_data;
    reg signed [BITWIDTH_DATA-'d1:0] taps_fir [2*LENGTH-'d2:0];
    reg [BITWIDTH_DATA+BITWIDTH_WEIGHTS-'d1:0] pre_out;
    ...
    assign DATA_OUT = {uint_io ^ pre_out[UPPER_MASK-'d1], pre_out[(UPPER_MASK-'d2)-:BITWIDTH_DATA-'d1]};   
    //One Clock - Multiplier
    assign mul_ina = ((state == STATE_FIR_FORW) || (state == STATE_FIR_BACK)) ? taps_fir[cnt_adr_data] : 'd0;
    assign mul_inb = ((state == STATE_FIR_FORW) || (state == STATE_FIR_BACK)) ? coeff_b[cnt_adr_wght] : 'd0;
    ...
    always@(posedge CLK) begin
        if(~(nRST && EN)) begin
            ...
            cnt_adr_data <= 'd0;
            cnt_adr_wght <= 'd0;
            for(i0 = 0; i0 < 2*LENGTH-'d1; i0 = i0 + 'd1) begin
                taps_fir[i0] = 'd0;
            end
        end else begin
            case(state)
    ...
    always@(negedge CLK) begin
        if(~nRST || (state == STATE_PREP)) begin
            ...
            pre_out <= pre_out + (((state == STATE_FIR_FORW) || (state == STATE_FIR_BACK)) ? mul_out : 'd0);
        end
    end    
