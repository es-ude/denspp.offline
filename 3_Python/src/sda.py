...
class SDA:
    def __init__(self, setting: SettingsSDA):
        ...
        self.frame_neg = round(self.settings.t_frame_start * self.settings.fs)
    ...
    def sda(self, xin: np.ndarray, mode: int) -> [np.ndarray, np.ndarray]:
        ksda = np.array(self.settings.dx_sda)
        if ksda.size == 0:
            x_mteo = xin
        else:
    ...
    def __movmean(self, xin: np.ndarray, window: int) -> np.ndarray:
        """Performing moving average on applied input"""
