...
def LoadSpAIke_Data(path2file: str, data_set: int, use_fulldata: bool, align_mode: int, separate_files: bool):
    settings = Settings()
    afe = AFE(settings)

    # --- Selection of datasets and points
    # TODO: Einfachere Struktur mit Settings setzen/erstellen
    ...
    for runSet in TakeDatasets:
        ...
        if separate_files:
            ...
            newfile_name = path2file + '_File' + runSet.astype('str')
            print('Saving file in: ' + newfile_name + '.mat')
            savemat(newfile_name + '.mat', matdata)
            # np.savez(newfile_name + '.npz', frames_in, frames_cluster)
            runs = 0
            frames_in = np.empty(shape=(0, 0), dtype=int)
            frames_cluster = np.empty(shape=(0, 0), dtype=int)
...
if __name__ == "__main__":
    ...
    separate_files = True
    path2save = 'data'
    file_name = str_date + '_Pedeira2012'
    path2file = os.path.join(path2save, file_name)
