...
class DataLoader(_DataController):
    ...
    def __load_method00_kirchner2023(self) -> None:
        ...
        loaded_data_preload = self._read_csv_file(self._path2file, 10, " ")
        ...
        path2data = join(self._settings.path, folder_name, marker_type)
        ...
        self._raw_data.data_type = "Orthese"
        self._raw_data.data_fs_orig = int(1000)
        # Electrode Mapping Design
        self._raw_data.data_mapping_avai = False
        self._raw_data.mapping_dimension = [1, 10]
        # Raw data
        self._raw_data.data_raw = [1.0 * data_used[idx, :] for idx in range(data_used.shape[0] - 1)]
        self._raw_data.data_time = self._raw_data.data_raw[0].shape[0] / self._raw_data.data_fs_orig
        # Groundtruth
        self._raw_data.label_exist = True
        self._raw_data.evnt_xpos = [np.array(loaded_marker) - data_used[-1, 0] for idx in range(data_used.shape[0] - 1)]
        self._raw_data.evnt_cluster_id = loaded_type
        self._raw_data.evnt_dict = loaded_type_dict
    ...
    def __load_method01_kirchner2024(self) -> None:
        ...
        data_type = '*set*.csv'
        self._prepare_access_file(folder_name, data_type)
        data_preloaded = self._read_csv_file(self._path2file, 8, ",")
        ...
        # Input and meta
        self._raw_data.data_name = folder_name
        self._raw_data.data_type = "Myo_Armband"
        self._raw_data.data_fs_orig = int(1000)
        # Electrode Design Information
        self._raw_data.data_mapping_avai = False
        self._raw_data.mapping_dimension = [1, 8]
        # Raw data
        self._raw_data.data_raw = [1.0 * data_used[idx, :] for idx in range(data_used.shape[0])]
        self._raw_data.data_time = self._raw_data.data_raw[0].shape[0] / self._raw_data.data_fs_orig
        # Groundtruth
