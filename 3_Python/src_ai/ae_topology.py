...
class dnn_ae_v1(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            nn.Tanh(),
            nn.Linear(in_features=iohiddenlayer[3], out_features=iohiddenlayer[2]),
            nn.Tanh(),
...
class cnn_dae_v1(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.BatchNorm1d(num_features=iohiddenlayer[0]),
            nn.Conv1d(in_channels=iohiddenlayer[0], out_channels=iohiddenlayer[1], kernel_size=kernelC[0],
                      padding='same'),
            nn.MaxPool1d(kernel_size=kernelM[0], padding='same'),
            nn.Tanh(), nn.BatchNorm1d(num_features=iohiddenlayer[1]),
            nn.Conv1d(in_channels=iohiddenlayer[1], out_channels=iohiddenlayer[2], kernel_size=kernelC[1],
                      padding='same'),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Conv1d(in_channels=iohiddenlayer[2], out_channels=iohiddenlayer[1], kernel_size=kernelC[0],
                      padding='same'),
            nn.Upsample(scale_factor=kernelM[1]),
            nn.Tanh(), nn.BatchNorm1d(num_features=iohiddenlayer[1]),
            nn.Conv1d(in_channels=iohiddenlayer[1], out_channels=iohiddenlayer[0], kernel_size=kernelC[1],
                      padding='same'),
