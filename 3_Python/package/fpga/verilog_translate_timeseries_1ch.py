...
def create_template_testbench(path2save='data/fpga_sim', output_bitsize=12) -> None:
    ...
    # Running code
    create_testbench(path2save=path2save)
...
def translate_data_memory(raw_data: np.ndarray, output_bitsize: int,
    ...
    with open(join(path2save, filename_mod + '.v'), 'w') as v_handler:
        ...
        v_handler.write(f'\tparameter DATA_LGTH = \'d{size_cnt},\n')
        v_handler.write(f'\tparameter DATA_BITSIZE = \'d{output_bitsize}\n)(\n')
        v_handler.write(f'\tinput wire CLK_ADC,\n')
        v_handler.write(f'\tinput wire nRST,\n')
        v_handler.write(f'\tinput wire EN,\n')
        v_handler.write(f'\toutput wire signed [DATA_BITSIZE-\'d1:0] DATA_OUT,\n')
        if trigger_avaible:
            v_handler.write(f'\toutput wire TRGG_OUT,\n')
        v_handler.write(f'\toutput wire DATA_END\n')
        v_handler.write(f');\n\n')
        v_handler.write(f'reg [$clog2(DATA_LGTH)-\'d1:0] cnt_pos;\n\n')
        if trigger_avaible:
            v_handler.write(f'reg bram_trgg [0:DATA_LGTH-\'d1];\n')
            v_handler.write(f'assign TRGG_OUT = (EN && !DATA_END) ? bram_trgg[cnt_pos] : 1\'d0;\n')
        v_handler.write(f'reg signed [DATA_BITSIZE-\'d1:0] bram_data [0:DATA_LGTH];\n')
        v_handler.write(f'assign DATA_OUT = (EN && !DATA_END) ? bram_data[cnt_pos] : \'d0;\n\n')
        v_handler.write(f'assign DATA_END = (cnt_pos == DATA_LGTH);\n\n')
        ...
        v_handler.write(f'\t\tcnt_pos = \'d0;\n')
        v_handler.write(f'\t\tfirst_start = 1\'d0;\n')
        v_handler.write(f'\tend else begin\n')
        v_handler.write(f'\t\tcnt_pos = cnt_pos + ((first_start && EN && !DATA_END) ? \'d1 : \'d0);\n')
        v_handler.write(f'\t\tfirst_start = EN;\n')
        v_handler.write(f'\tend\n')
        v_handler.write(f'end\n\n')
