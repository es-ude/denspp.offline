...
%     'Marker', '.', ...
%     'MarkerSize', 10);
% plot(x, Tano_id, 'r--', 'Linewidth', line);
% ylim([76 82]);
% xlim([0 xStimPlot]);
% 
% setGraphicStyle(1);
% ylabel('No. of clocks n_{ano}');
% xlabel('No. of Stim.-Phases');
% legend({'\Delta Q (wo ChBLC)';'\Delta Q (w ChBLC)';'n_{ano}'}, 'Location', 'SouthEast');
...
% y_axis = ylim;
% dy = diff(ylim)/16;
% text(x_axis(1)+dx, y_axis(2)-dy,        ['Mean =' num2str(round(mean(dQcatM),2))]);
% text(x_axis(1)+dx, y_axis(2)-dy*(1.6),  ['Std =' num2str(round(std(dQcatM),2))]);
% 
% subplot(1,2,2);
% histfit(dQanoM, 20);
% grid on;
...
% y_axis = ylim;
% dy = diff(ylim)/16;
% text(x_axis(1)+dx, y_axis(2)-dy,        ['Mean =' num2str(round(mean(dQanoM),2))]);
% text(x_axis(1)+dx, y_axis(2)-dy*(1.6),  ['Std =' num2str(round(std(dQanoM),2))]);
% 
% % --- Speichern
...
%Qre = 0.6*Qre;
hist(Qre, 60, 'FaceColor', [0.502 0.502 0.502]);
grid on;
xlabel('\Delta Q / pC');
setGraphicStyle(1);
ylim([0 140]);
...
y_axis = ylim;
dy = abs(diff(ylim)/8);
text(x_axis(1)+dx, y_axis(2)-dy*0.5,  ['Mean =' num2str(round(mean(Qre),2))], 'FontSize', 16);
text(x_axis(1)+dx, y_axis(2)-dy*1,  ['Std =' num2str(round(std(Qre),2))], 'FontSize', 16);

savefig(h, [PathFig 'Histo_dQ.fig']);
saveas(h, [PathFig 'Histo_dQ.eps'], 'eps');
saveas(h, [PathFig 'Histo_dQ.jpg'], 'jpg');

h = figure(4);

histfit(Tano_re,1+(max(Tano_re)-min(Tano_re)));
grid on;
xlabel('n_{ano}');
setGraphicStyle(1);

savefig(h, [PathFig 'Histo_nAno.fig']);
saveas(h, [PathFig 'Histo_nAno.eps'], 'eps');
saveas(h, [PathFig 'Histo_nAno.jpg'], 'jpg');
