...
class SinePulseStimulator(NgSpiceShared):
    def __init__(self, waveform, amplitude, f_samp, **kwargs):
        super().__init__(**kwargs)
        self._waveform = waveform
        self._amplitude = float(amplitude)
        self._f_samp = float(f_samp)

    def get_vsrc_data(self, voltage, time, node, ngspice_id):
        self._logger.debug('ngspice_id-{} get_vsrc_data @{} node {}'.format(ngspice_id, time, node))
        voltage[0] = self._amplitude * self._waveform[int(time*float(self._f_samp)) % len(self._waveform)]
        return 0

    def get_isrc_data(self, current, time, node, ngspice_id):
        self._logger.debug('ngspice_id-{} get_isrc_data @{} node {}'.format(ngspice_id, time, node))
        current[0] = self._amplitude * self._waveform[int(time*float(self._f_samp)) % len(self._waveform)]
        return 0
