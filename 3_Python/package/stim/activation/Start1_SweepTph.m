...
%% --- Header
    ...
    %                   Stärke-Dauer-Beziehung (Phasendauer)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp('Stärke-Dauer-Beziehung des 2D-Modells (Phasendauer)');
disp('-----------------------------------------------------------');
...
TphS = 1e-3*logspace(-2, 1, 106);    % Phasendauer [s]
...
ModulationStim = 2;                 % 1: VCS, 2: CCS, 3: ChCS
...
dT0 = 0.5e-3;               % Zeitpunkt der Stimulation [s]
...
noIte = zeros(size(TphS));
Qth = zeros(size(TphS));
Eth = zeros(size(TphS));
Ith = zeros(size(TphS));
PST = zeros(size(TphS));
fth = zeros(size(TphS));
fChCS = 10.^(1+floor(log10(1./TphS)));
    ...
    while IterationDone == 0
        ...
        if((isempty(VmaxAP) == 1-cntDir) || (isempty(VmaxAP) == 2+cntDir))
            ...
            if(IterationNo > IterationRange)||(IterationNo+IterationDeltaNo > length(Iteration))
                ...
                PST(k) = t(x0)-dT0;
                IterationDone = 1;
                disp(['.. Step ', num2str(k), ' of ', num2str(length(TphS)), ' done!']);
            else
                if(ModulationStim == 3)
        ...
        if(IterationCycle >= 500)
            ...
            PST(k) = 1;
            IterationDone = 1;
            disp(['.. Step ', num2str(k), ' of ', num2str(length(TphS)), ' aborted!']);
        end
    end
    disp(['.. Step ', num2str(k), ' of ', num2str(length(TphS)), ' done!']);
end
...
% linearer Fit der fehlenden Stellen
if(ModulationStim == 3)
    fth(1) = fth(2) + (fth(2) - fth(3));
    Qth(1) = Qth(2) + (Qth(2) - Qth(3));
    A = find(Qth == 0);
    if(isempty(A) == 0)
        for i=1:1:length(A)
            if(A(i) == length(Qth))
                Qth(A(i)) = Qth(A(i) + (-1)^floor(i/length(A)));
            else
                if(A(i) == 1)
                else
                    Qth(A(i)) = 0.5*(Qth(A(i)+1) +  Qth(A(i)-1));
                end
            end
        end
    end
...
% --- Ermitteln der Chronaxie-Rheobase-Parameter
% I_rheo = Ith(end);
% A = find(Ith >= 2* I_rheo);
% T_chro = 2*TphS(A(end));
% clear A;
% Iph0 = I_rheo.*(1 + T_chro./TphS);

disp('-----------------------------------------------------');
disp('Auswertung');
disp(['Axon-Widerstand: ',      num2str(1e-3*Raxon),        ' kOhm']);
disp(['Medium-Widerstand: ',    num2str(1e-3*max(Rmed)),    ' kOhm']);
%disp(['Rheobase-Strom: '        num2str(1e6*I_rheo),         ' uA']);
%disp(['Chronaxie-Dauer: '       num2str(1e6*T_chro),         ' us']);
...
figure(1);
yyaxis left;
loglog(1e3* TphS, corFac*Ith, 'k', 'Linewidth', line);
hold on;  
loglog(1e3* TphS, 1e9*Eth, 'r', 'Linewidth', line);
ylabel(YString1);
ylim([YLim(1) YLim(end)]);
yticks(YLim);

yyaxis right;
loglog(1e3* TphS, 1e9*abs(Qth), 'b', 'Linewidth', line);
hold on;
%loglog(1e3* TphS, 1e3*PST, 'm', 'Linewidth', line);
...
FileName = [FileName TextWFG{patternWFG} '_d' num2str(xyNode(1)*1e6) 'um'];
...
save([PathData FileName '.mat'], 'Tipd', 'ModulationStim', 'firstStim', 'phaseWFG', 'patternWFG', 'T', 'xyNode', 'Raxon', 'LAxon', 'noNode', 'Cmod', 'Cdl', 'Rtis', 'Rfar', 'TphS', 'Ith', 'Qth', 'Eth');

%% --- Plotten - Figure 2 (PST)
figure(2);
semilogx(1e3* TphS, 1e3*PST, 'k', 'Linewidth', line);
setGraphicStyle(1);
