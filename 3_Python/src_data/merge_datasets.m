...
frames_bad = 0 * frames_cluster;
for idx = (1:1:size(unique(frames_cluster)))
    test_plot(frames_in, frames_cluster, idx-1);
    pause(2);
    sel_id = input("And?");
    
    sel = find(frames_cluster == idx-1);
    frames_bad(sel) = sel_id;
    close;
end

%% --- Saving output
save(setOptions.path2save, 'frames_in', 'frames_cluster', 'frames_bad');
clear FileName;
