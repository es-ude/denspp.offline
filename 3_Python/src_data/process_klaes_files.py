...
class KlaesHandler:
    ...
    def search_data(self, sel_folder: int) -> dict:
        ...
        files_behv.sort()
        files_sorted = glob.glob(os.path.join(path2data, folder_content[sel_folder], type_sorted))
        files_sorted.sort()
        ...
        dataset_path['Sorted'] = path2sorted
        ...
        if len(path2sorted) != 0:
            dataset_path['Exits_Sorted'] = True
        else:
            dataset_path['Exits_Sorted'] = False
    ...
    def process_label_mat(self, data_dict: dict, sel_file: int) -> dict:
        ...
        if not os.path.exists(path2file):
            ...
            try:
                ...
                groundtruth_old1 = scipy_loadmat(path2file)
    ...
    # TODO: Routine entwickeln, die die sorted aus *.mat oder *sorted*.mat holt
    def process_label_sorted(self, data_dict: dict, sel_file: int) -> dict:
        # Process '*sorted*.mat'-File from SpikeDeepClassifier
        groundtruth_klaes = dict()

        if not data_dict['Exits_Sorted']:
            groundtruth_klaes['Exits'] = False
        else:
            path2file = data_dict['Sorted'][sel_file]
            if not os.path.exists(path2file):
                groundtruth_klaes['Exits'] = False
            else:
                groundtruth_klaes['Exits'] = True
                sorted = scipy_loadmat(path2file)
                groundtruth_klaes.update({'sorted': sorted['sortedDataSession']})

        return groundtruth_klaes
    ...
    def save_results(self, name: str, rawdata: dict, label: dict, sorted: dict, behaviour: dict):
        mdic = {'rawdata': rawdata, 'nev_detected': label, 'sorted': sorted, 'behaviour': behaviour}
